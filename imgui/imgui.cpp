...
static ImVec2 CalcSizeContents(ImGuiWindow* window) {
  if (window->Collapsed) return window->SizeContents;
...
bool ImGui::Begin(const char* name, bool* p_open, ImGuiWindowFlags flags) {
        ...
        ImVec2 size_for_clamping =
            ((g.IO.ConfigWindowsMoveFromTitleBarOnly) &&
             !(window->Flags & ImGuiWindowFlags_NoTitleBar))
                ...
                : window->Size;
        window->Pos =
            ImMax(window->Pos + size_for_clamping, padding) - size_for_clamping;
  ...
  window->Hidden =
      (window->HiddenFramesRegular > 0) || (window->HiddenFramesForResize > 0);
...
void ImGui::ClosePopupsOverWindow(ImGuiWindow* ref_window) {
  ...
  if (ref_window) {
    ...
    for (; popup_count_to_keep < g.OpenPopupStack.Size; popup_count_to_keep++) {
      ...
      if (!popup_or_descendent_has_focus) break;
    }
  }
