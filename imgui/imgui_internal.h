...
enum ImGuiTabBarFlagsPrivate_ {
  ImGuiTabBarFlags_DockNode =
      1 << 20,  // [Docking: Unused in Master Branch] Part of a dock node
  ImGuiTabBarFlags_DockNodeIsDockSpace =
      1 << 21,  // [Docking: Unused in Master Branch] Part of an explicit
                // dockspace node node
...
struct ImGuiTabBar {
  ...
  int GetTabOrder(const ImGuiTabItem* tab) const {
    return Tabs.index_from_pointer(tab);
  }
};
...
IMGUI_API void RenderPixelEllipsis(ImDrawList* draw_list, ImFont* font,
                                   ImVec2 pos, int count, ImU32 col);
...
extern void ImGuiTestEngineHook_ItemInfo(ImGuiID id, const char* label,
                                         int flags);
#define IMGUI_TEST_ENGINE_ITEM_INFO(_ID, _LABEL, _FLAGS) \
  ImGuiTestEngineHook_ItemInfo(_ID, _LABEL, _FLAGS)  // Register status flags
#else
#define IMGUI_TEST_ENGINE_ITEM_INFO(_ID, _LABEL, _FLAGS) \
  do {                                                   \
  } while (0)
