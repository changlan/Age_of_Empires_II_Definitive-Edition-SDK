...
bool ImGui::ButtonBehavior(const ImRect& bb, ImGuiID id, bool* out_hovered,
  ...
  if (window->DC.LastItemId != id) ImGuiTestEngineHook_ItemAdd(bb, id);
#endif
...
bool ImGui::BeginTabBarEx(ImGuiTabBar* tab_bar, const ImRect& tab_bar_bb,
  ...
  const float y = tab_bar->BarRect.Max.y - 1.0f;
  {
    const float separator_min_x =
        tab_bar->BarRect.Min.x - ((flags & ImGuiTabBarFlags_DockNodeIsDockSpace)
                                      ? 0.0f
                                      : window->WindowPadding.x);
    const float separator_max_x =
        tab_bar->BarRect.Max.x + ((flags & ImGuiTabBarFlags_DockNodeIsDockSpace)
                                      ? 0.0f
                                      : window->WindowPadding.x);
...
static void ImGui::TabBarScrollToTab(ImGuiTabBar* tab_bar, ImGuiTabItem* tab) {
  ...
  if (tab_bar->ScrollingTarget > tab_x1) tab_bar->ScrollingTarget = tab_x1;
...
bool ImGui::TabItemLabelAndCloseButton(ImDrawList* draw_list, const ImRect& bb,
  ...
  if (label_size.x > text_ellipsis_clip_bb.GetWidth()) {
    ...
    if (!close_button_visible && ellipsis_x + ellipsis_width <= bb.Max.x)
      RenderPixelEllipsis(draw_list, g.Font,
