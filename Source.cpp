...
typedef HRESULT(__stdcall* D3D11PresentHook)(IDXGISwapChain* pSwapChain,
...
HRESULT __stdcall hookD3D11Present(IDXGISwapChain* pSwapChain,
                                   UINT SyncInterval, UINT Flags) {
  if (firstTime) {
    ...
    ImGui::GetIO().WantCaptureMouse || ImGui::GetIO().WantTextInput ||
        ImGui::GetIO().WantCaptureKeyboard;  // control menu with mouse
    io.ConfigFlags |=
    ...
    pDevice->CreateDepthStencilState(&depthStencilDesc,
                                     &DepthStencilState_FALSE);
...
LRESULT CALLBACK DXGIMsgProc(HWND hwnd, UINT uMsg, WPARAM wParam,
                             LPARAM lParam) {
...
DWORD __stdcall InitHooks(LPVOID hModule) {
  ...
  WNDCLASSEXA wc = {sizeof(WNDCLASSEX),
                    ...
                    NULL};
  RegisterClassExA(&wc);
  HWND hWnd = CreateWindowA("DX", NULL, WS_OVERLAPPEDWINDOW, 100, 100, 300, 300,
  ...
  if (FAILED(D3D11CreateDeviceAndSwapChain(
          ...
          D3D11_SDK_VERSION, &scd, &pSwapChain, &pDevice, &obtainedLevel,
          &pContext))) {
    MessageBoxA(hWnd, "Failed to create directX device and swapchain!", "Error",
                MB_ICONERROR);
    return NULL;
  }
