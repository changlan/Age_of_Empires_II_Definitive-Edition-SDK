...
Core::Core() {
  onCreateUnitHook.Hook((BYTE*)GetModuleHandle(NULL) + Offsets::createUnitHook,
                        (BYTE*)OnCreateUnitHook, 15);
...
void createPlayerTreeNode(Player* player, int playerIndex) {
  ImGui::PushStyleColor(ImGuiCol_Text,
                        Engine::Get()->GetPlayerColorImGUI(*player->pColor));

  std::string playerText = "Player " + std::to_string(playerIndex);
  if (ImGui::TreeNode(player->name)) {
    ImGui::PushStyleColor(ImGuiCol_Text, 0xffffffff);
    ImGui::Text("Player %p (%f, %f)", player, player->xScreenPos,
                player->yScreenPos);
    FeatureManager::Get()->OnMenuPlayerTreenode(player, playerIndex);
    if (ImGui::TreeNode(player->pObjectManager->units, "Units %p",
                        (int64_t)player->pObjectManager->units)) {
      int buildingCount = 0;
      int infantryCount = 0;
      int calavaryCount = 0;
      for (int i = 0; i < player->pObjectManager->Count; i++) {
        ...
        if (unit->GetOwner() == player) {
          ...
          ImGui::Text("%f %f %f Health: %f", unit->GetPosition().x,
                      unit->GetPosition().y, unit->GetPosition().z,
                      unit->GetHealth());
          ImGui::SameLine();
          // std::string text = std::string("Copy##") +
          // std::string(unit->GetUnitData()->GetName()) + std::to_string(i);

          if (ImGui::Button("Copy")) {
            Utility::CopyToClipboard((uint64_t)unit);
          }
          /*std::string vmtTree = std::string("VMT##") +
          std::string(unit->pUnitData->name) + "VMT"; if
          (ImGui::TreeNode(vmtTree.c_str()))
          {
                  for (int vmtIndex = 0; vmtIndex < 175; vmtIndex++)
                  {
                          std::string vmtText = std::string("Copy##") +
          std::string(unit->pUnitData->name) + std::to_string(vmtIndex) + "VMT";
                          ImGui::Text("%d - %p", vmtIndex,
          *(int64_t**)(*(int64_t**)unit + vmtIndex)); ImGui::SameLine(); if
          (ImGui::Button(vmtText.c_str()))
                          {
                                  Utility::CopyToClipboard((uint64_t) *
          (int64_t**)(*(int64_t**)unit + vmtIndex));
                          }
                  }
                  ImGui::TreePop();
          }*/
...
void Core::OnPresent() {
  __try {
    ...
    if (openOverlay) {
      __try {
        if (ImGui::Begin("Age of Empires 2 DE", &openOverlay,
                         ImGuiWindowFlags_NoSavedSettings |
                             ImGuiWindowFlags_NoFocusOnAppearing |
                             ImGuiWindowFlags_NoNav)) {
          if (ImGui::TreeNode("Debug")) {
            ImGui::Text("World %p", world);
            ImGui::Text("Map %p tilesize %d", world->pMap,
                        world->pMap->GetTileSize());
            ...
            ImGui::Text("ScreenPos %p %f %f %f",
                        mainScreen->pGameScreen->pMainView,
                        mainScreen->pGameScreen->pMainView->ScreenPosX,
                        mainScreen->pGameScreen->pMainView->ScreenPosY,
                        mainScreen->pGameScreen->pMainView->ScreenPosZ);
            ImGui::TreePop();
          }
          ImGui::Separator();
          ...
          for (int i = 0; i < totalPlayers; i++) {
            ...
            if (skipLocalplayer &&
                currentPlayer == Engine::Get()->GetLocalPlayer()) {
              continue;
            }
...
void Core::OnShutdown() { onCreateUnitHook.Unhook(); }
