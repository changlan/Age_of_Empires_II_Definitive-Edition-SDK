...
VmtHook::VmtHook(void** vmt) { this->vmt = vmt; }
...
void* VmtHook::Hook(int index, void* hookedFunction) {
  ...
  VirtualProtect(&vmt[index], sizeof(BYTE*), PAGE_EXECUTE_READWRITE,
                 &pageProtection);
...
void VmtHook::Unhook() {
  for (std::pair<int, void*> pair : hookedfuncs) {
    DWORD oldProtection;
    VirtualProtect(&vmt[pair.first], sizeof(BYTE*), PAGE_EXECUTE_READWRITE,
                   &oldProtection);
    vmt[pair.first] = pair.second;
    VirtualProtect(&vmt[pair.first], sizeof(BYTE*), oldProtection,
                   &oldProtection);
  }
}