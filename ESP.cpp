...
uint32_t ESP::colors_hex[8] = {0xff0000ff, 0xffff0000, 0xff00ff00, 0xffffff00,
                               0xff00ffff, 0xffff00ff, 0xffffffff, 0xffffb400};
...
void ESP::DrawBox(Unit* unit, int32_t color, bool drawName = false) {
  ...
  if (drawName) {
    ...
    Renderer::Get()->RenderText(std::string(unit->GetUnitData()->GetName()),
                                ivTextPos, 16, color, false);
  }
}
...
void ESP::DrawCircle(Unit* unit, int radius, int32_t color, int smoothness = 16,
                     float thickness = 1.f, bool drawName = false) {
  ...
  for (int i = 1; i < screeenPoints.size(); i++) {
    Renderer::Get()->RenderLine(screeenPoints[i], screeenPoints[i - 1], color,
                                thickness);
  }
  Renderer::Get()->RenderLine(screeenPoints[0],
                              screeenPoints[screeenPoints.size() - 1], color,
                              thickness);
  if (drawName) {
    Vector2 screenTextPos = Engine::Get()->worldToScreen(center);
    ImVec2 ivTextPos = ImVec2(screenTextPos.x, screenTextPos.y);
    Renderer::Get()->RenderText(unit->GetUnitData()->GetName(), ivTextPos, 16,
                                color, false);
  }
}
...
void ESP::OnUnitIteration(Unit* unit, Player* player, int playerIndex) {
  if (playerUnitEsp[playerIndex]) {
    ...
    if (siegeImpactLocation) {
      if (std::string(unit->GetUnitData()->GetName())
              .find("Projectile Scorpion") != std::string::npos) {
        Vector3* destination = unit->GetTargetPosition();
        if (destination) {
          Vector2 screenDestinationPos =
              Engine::Get()->worldToScreen(*destination);
          Renderer::Get()->RenderCircleFilled(
              ImVec2(screenDestinationPos.x, screenDestinationPos.y), 10,
              colors_hex[*player->pColor] & 0x77ffffff);
        }
      }

      if (std::string(unit->GetUnitData()->GetName())
              .find("Projectile Mangonel") != std::string::npos) {
        Vector3* destination = unit->GetTargetPosition();
        if (destination) {
          Vector2 screenDestinationPos =
              Engine::Get()->worldToScreen(*destination);
      ...
      if (std::string(unit->GetUnitData()->GetName())
              .find("Projectile Trebuchet") != std::string::npos) {
        Vector3* destination = unit->GetTargetPosition();
        if (destination) {
          Vector2 screenDestinationPos =
              Engine::Get()->worldToScreen(*destination);
          Renderer::Get()->RenderCircleFilled(
              ImVec2(screenDestinationPos.x, screenDestinationPos.y), 30,
              colors_hex[*player->pColor] & 0x77ffffff);
        }
      }
    }
    ...
    */
    ...
    if (trebuchetESP &&
        (std::string(unit->GetUnitData()->GetName()).find("TREBU") !=
             std::string::npos ||
         std::string(unit->GetUnitData()->GetName()).find("PTREB") !=
             std::string::npos)) {
    ...
    if (playerUnitDestinationEsp[playerIndex]) {
      ...
      Renderer::Get()->RenderLine(ImVec2(screenPos.x, screenPos.y),
                                  ImVec2(screenTargetPos.x, screenTargetPos.y),
                                  colors_hex[*player->pColor]);
    }
  }
}
...
void ESP::OnNeutralUnit(Unit* unit) {
  if (gaiaESP || goldESP || stoneESP) {
    ...
    if (stoneESP && strcmp(unitName.c_str(), "FORAG") == 0 ||
        stoneESP && strcmp(unitName.c_str(), "FORAGM") == 0) {
    ...
    if (std::find(namesFish.begin(), namesFish.end(), unitName) !=
        namesFish.end()) {
      Renderer::Get()->RenderCircleFilled(ImVec2(screenPos.x, screenPos.y), 20,
                                          0x400000ff);
      Renderer::Get()->RenderText(unitName, ImVec2(screenPos.x, screenPos.y),
                                  16, 0xffffffff);
      return;
    }

    if (std::find(namesHeavyFood.begin(), namesHeavyFood.end(), unitName) !=
        namesHeavyFood.end()) {
      Renderer::Get()->RenderCircleFilled(ImVec2(screenPos.x, screenPos.y), 20,
                                          0x4000ff00);
      Renderer::Get()->RenderText(unitName, ImVec2(screenPos.x, screenPos.y),
    ...
    if (std::find(namesLighthuntable.begin(), namesLighthuntable.end(),
                  unitName) != namesLighthuntable.end()) {
      Renderer::Get()->RenderCircleFilled(ImVec2(screenPos.x, screenPos.y), 20,
                                          0x4000ffff);
      Renderer::Get()->RenderText(unitName, ImVec2(screenPos.x, screenPos.y),
                                  16, 0xffffffff);
    ...
    if (std::find(namesLightFood.begin(), namesLightFood.end(), unitName) !=
        namesLightFood.end()) {
      Renderer::Get()->RenderCircleFilled(ImVec2(screenPos.x, screenPos.y), 20,
                                          0x400000ff);
      Renderer::Get()->RenderText(unitName, ImVec2(screenPos.x, screenPos.y),
    ...
    if (std::find(namesDangerousAnimal.begin(), namesDangerousAnimal.end(),
                  unitName) != namesDangerousAnimal.end()) {
      Renderer::Get()->RenderCircleFilled(ImVec2(screenPos.x, screenPos.y), 20,
                                          0x40ff0000);
      Renderer::Get()->RenderText(unitName, ImVec2(screenPos.x, screenPos.y),
                                  16, 0xffffffff);
    ...
    if (strcmp(unitName.c_str(), "RELIC") == 0) {
      Renderer::Get()->RenderCircleFilled(ImVec2(screenPos.x, screenPos.y), 50,
                                          0x40ffffff);
      Renderer::Get()->RenderText(unitName, ImVec2(screenPos.x, screenPos.y),
                                  16, 0xffffffff);
    }
