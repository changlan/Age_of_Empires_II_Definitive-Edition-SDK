...
void __fastcall minimapProxy(Registers* registers) {
  ...
  if (!displayWood && !displayFood && !displayGold && !displayStone &&
      !displayPopulation) {
    return;
  }
  ...
  std::string populationString =
      std::to_string((int)player->pResources->currentPop) + "/" +
      std::to_string((int)player->pResources->currentPop +
                     (int)player->pResources->popSpaceLeft);
  ...
  MidfunctionHook::OverwriteRegister(registers->rsp, Register::RDI,
                                     (int64_t)newName.c_str());
}
...
void MinimapText::OnInitialise() {
  ...
  minimapHook.Hook(
      (BYTE*)(int64_t)GetModuleHandle(NULL) + Offsets::minimapHookOffset,
      (BYTE*)minimapProxy, 20);
}

void MinimapText::OnShutdown() { minimapHook.Unhook(); }
