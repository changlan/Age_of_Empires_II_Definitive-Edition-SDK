...
void CastleManager::OnUnitIteration(Unit* unit, Player* player,
                                    int playerIndex) {}
...
void CastleManager::OnUnitCreated(Unit* unit) {
  ...
  if (strcmp("CSTL", unit->GetUnitData()->GetName()) == 0) {
    std::string message =
        std::string(owningPlayer->name) + " is building a castle!";
...
void CastleManager::OnDraw() {
  /*if (strcmp("CSTL", unit->pUnitData->name) == 0)
  {
          ...
          int screenResX =
  Engine::Get()->GetMainScreen()->pGameScreen->ScreenResX; int screenResY =
  Engine::Get()->GetMainScreen()->pGameScreen->ScreenResY;
          Renderer::Get()->RenderLine(ImVec2(screenResX / 2, screenResY / 2),
  ImVec2(screenPos.x, screenPos.y), 0xffffffff, 2);
  }*/
}
