...
// This function is required because changes like: registers->rdi = 0xdeadbeef
// will get restored by the trampoline (pop rdi) To change the value pointed to
// by a register just dereference it instead: *(int64*)registers->rdi = 0x4711
// (usage of this function not required)
void MidfunctionHook::OverwriteRegister(int64_t rsp, Register reg,
                                        int64_t value) {
  *((int64_t*)rsp + (int)reg) = value;
}

//
// Hooks a function anywhere by placing jumping to a trampoline. The trampoline
// saves all registers and then calls the proxy function 0xff, 0x25, 0x0, 0x0,
// 0x0, 0x0					JMP[rip + 0] 0x11, 0x22, 0x33,
// 0x44, 0x55, 0x66, 0x77, 0x88	absolute address of jump The proxy functions
// signature is expected to be: void __fastcall proxyFunction(registers*
// registers) Important: the overwritten bytes are NOT relocated meaning only
// position independet instructions can be overwritten
//
// TODO:
// When hooking within prologe or epicloge of a function the stack may not be
// aligned use BTR RSP, 3 to align stack and save RSP to then later restore it
void MidfunctionHook::Hook(BYTE* sourceAddress, BYTE* targetAddress,
                           const int hookLength) {
  ...
  BYTE stub[stubLength] = {
      ...
      0x48, 0x83, 0xEC, 0x28,              // sub    rsp,0x20
                               // (allocate shadow space)
      0x48, 0xB8, 0x88, 0x77, 0x66, 0x55,
      0x44, 0x33, 0x22, 0x11,  // movabs rax,0x1122334455667788		(use
                               // register to have an absolute 8 byte call)
      0xFF, 0xD0,  // call   rax						(call proxy
                   // function)
      0x48, 0x83, 0xC4, 0x28,  // add    rsp,0x20
  ...
  BYTE stubJumpBack[stubJumpBackLength] =
      {
          ...
          0x77, 0x88  // absolute address of jump
      };
  ...
  trampoline =
      (BYTE*)VirtualAlloc(NULL, stubLength + hookLength + stubJumpBackLength,
  ...
  *(int64_t*)&trampoline[stubLength + hookLength + 6] =
      (int64_t)&sourceAddress[hookLength];

  // make trampoline executable
  // DWORD pageProtection;
  // VirtualProtect(trampoline, stubLength + hookLength + stubJumpBackLength,
  // PAGE_EXECUTE_READWRITE, &pageProtection);
  ...
  *(uint64_t*)(&sourceAddress[2 + 4]) =
      (uint64_t)(trampoline);  // destination to jump to
...
void MidfunctionHook::Unhook() {
  ...
  // memory leak but enables unhooking inside hooked function and makes it
  // threadsafe? delete[] trampoline;
}
